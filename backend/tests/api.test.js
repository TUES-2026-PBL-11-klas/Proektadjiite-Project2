...
const mockGetUploadedFileUrl = vi.fn(() => 'http://localhost:3000/uploads/test-image.jpg');
...
describe('Reports endpoints', () => {
  ...
  it('GET /api/reports validates filters', async () => {
    const response = await request(app).get('/api/reports').query({ status: 'bad_status' });
  ...
  it('GET /api/reports returns report list with pagination', async () => {
    ...
    const response = await request(app).get('/api/reports').query({ limit: 10, offset: 0 });
...
describe('Votes, status updates, and admin analytics', () => {
  it('POST /api/reports/:id/vote records a vote', async () => {
    ...
    mockPrisma.report.update.mockResolvedValueOnce({ id: reportId, vote_count: 1 });
