...
import { MapPin, ZoomIn, ZoomOut, Locate, Layers, Navigation, Compass } from 'lucide-react'
...
const statusColors: Record<string, { bg: string; text: string; glow: string }> = {
  submitted: { bg: 'bg-amber-500', text: 'text-amber-500', glow: 'shadow-amber-500/50' },
  in_progress: { bg: 'bg-blue-500', text: 'text-blue-500', glow: 'shadow-blue-500/50' },
  resolved: { bg: 'bg-emerald-500', text: 'text-emerald-500', glow: 'shadow-emerald-500/50' },
  archived: { bg: 'bg-gray-400', text: 'text-gray-400', glow: 'shadow-gray-400/50' },
}
...
export function MapPlaceholder({
  ...
  return (
    <div className={cn('relative bg-surface rounded-2xl overflow-hidden shadow-inner', className)}>
      {/* Map background */}
      <div 
        className={cn(
          'absolute inset-0',
      ...
      >
        ...
        >
          ...
          {/* Main roads */}
          <path d="M0,35 Q30,32 50,35 T100,30" stroke="rgba(26, 71, 49, 0.12)" strokeWidth="0.4" fill="none" />
          <path d="M0,55 Q40,58 60,55 T100,58" stroke="rgba(26, 71, 49, 0.12)" strokeWidth="0.4" fill="none" />
          <path d="M0,75 Q35,72 55,76 T100,73" stroke="rgba(26, 71, 49, 0.08)" strokeWidth="0.3" fill="none" />
          
          {/* Vertical roads */}
          <path d="M25,0 Q28,30 25,50 T28,100" stroke="rgba(26, 71, 49, 0.10)" strokeWidth="0.35" fill="none" />
          <path d="M50,0 Q48,40 52,60 T50,100" stroke="rgba(26, 71, 49, 0.12)" strokeWidth="0.4" fill="none" />
          <path d="M75,0 Q73,35 76,55 T74,100" stroke="rgba(26, 71, 49, 0.10)" strokeWidth="0.35" fill="none" />
          
          {/* Ring road (boulevard) */}
          <ellipse cx="50" cy="50" rx="30" ry="25" stroke="rgba(26, 71, 49, 0.14)" strokeWidth="0.5" fill="none" strokeDasharray="2,1" />
        </svg>
        ...
        <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full opacity-60" preserveAspectRatio="none">
          {/* South Park */}
          <ellipse cx="45" cy="65" rx="8" ry="5" fill="rgba(34, 197, 94, 0.15)" />
          {/* Borisova gradina */}
          <ellipse cx="70" cy="45" rx="10" ry="7" fill="rgba(34, 197, 94, 0.12)" />
          {/* Small parks */}
        ...
        <svg viewBox="0 0 200 40" className="absolute bottom-0 left-0 right-0 w-full h-20 opacity-[0.07]" preserveAspectRatio="none">
          <defs>
              ...
              <div className="relative group">
                ...
                <div 
                  className={cn(
                    ...
                    colors.bg,
                    isHovered || isSelected ? 'opacity-40 scale-150' : 'opacity-0'
                  )}
                />
                ...
                <div className={cn(
                  ...
                  isHovered || isSelected ? 'drop-shadow-lg' : 'drop-shadow-md'
                )}>
                  <MapPin 
      ...
      {showControls && (
        <div className="absolute bottom-24 right-4 flex flex-col bg-white/95 backdrop-blur-md rounded-xl shadow-lg border border-border/50 overflow-hidden">
          <button className="p-2.5 hover:bg-forest/8 transition-colors group border-b border-border/30">
            <ZoomIn size={17} className="text-muted-foreground group-hover:text-forest transition-colors" />
          </button>
          <button className="p-2.5 hover:bg-forest/8 transition-colors group border-b border-border/30">
            <ZoomOut size={17} className="text-muted-foreground group-hover:text-forest transition-colors" />
          </button>
          <button className="p-2.5 hover:bg-forest/8 transition-colors group border-b border-border/30">
            <Locate size={17} className="text-muted-foreground group-hover:text-forest transition-colors" />
          </button>
          <button className="p-2.5 hover:bg-forest/8 transition-colors group">
            <Layers size={17} className="text-muted-foreground group-hover:text-forest transition-colors" />
          </button>
        </div>
      )}
