...
import { mockReports, type ReportCategory, type ReportStatus, mockCurrentUser } from '@/lib/mock-data'
import { Button } from '@/components/ui/button'
import { Filter, List, Map as MapIcon, Activity, TrendingUp, CheckCircle2, Clock } from 'lucide-react'
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 relative">
              ...
              <div className="h-full overflow-y-auto p-4 space-y-3">
                {filteredReports.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="font-heading font-semibold text-lg mb-2">Няма намерени сигнали</h3>
            ...
            <div className="absolute top-4 right-4 hidden md:flex flex-col gap-3">
              {/* Main Stats Card */}
              <div className="bg-card/95 backdrop-blur-sm shadow-xl rounded-2xl p-5 border border-forest/10">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-medium text-muted-foreground">Активни сигнали</span>
                </div>
                <p className="text-4xl font-heading font-bold text-forest mb-4">
                  {filteredReports.filter(r => r.status !== 'archived').length}
                </p>
                <div className="grid grid-cols-3 gap-3 pt-4 border-t">
                  ...
                  <div className="text-center">
                    ...
                    <span className="text-xs text-muted-foreground">Решени</span>
                  </div>
                </div>
              </div>

              {/* Quick Stats */}
              <div className="bg-forest/95 backdrop-blur-sm shadow-xl rounded-2xl p-4 text-white">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <TrendingUp size={16} className="text-lime" />
                    <span className="text-sm font-medium text-white/80">Тази седмица</span>
                  </div>
                  <span className="text-lg font-bold text-lime">+12%</span>
                </div>
              </div>
            </div>
          </div>
